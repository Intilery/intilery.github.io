...
Here’s the payload of a typical `track` call with most [common fields](common) removed:
...
**Note: In ourwebsite tag and mobile libraries (coming soon) a User ID is automatically added** from the state stored by a previous [`identify`](identify/) call, so you do not need to add it yourself. They will also automatically handle Anonymous ID’s under the covers.
...
We have standardized a series of reserved event names that have special semantic meaning. See the [Semantic Events docs](intilery) for more detail.
...
The following is all the reserved properties we have standardized that apply to all events. Check out the [Semantic Events docs](intilery) for properties specific to individual reserved events.
